...
### Task 12: Meta description + CSP
- CSP будет добавлена: `script-src: 'self' https://cdn.tailwindcss.com https://fonts.googleapis.com', connect-src: 'self'`
- Цель: защитить от загрузки вредоносных скриптов

### Task 13: Форма CTA
- Валидация будет добавлена на инпуты при добавлении backend
- Цель: защитить от XSS при будущем функционале

### Task 17: OWASP Top 10 Review
- Будут проверены: XSS, HTML injection, аутентификация, шифрование
- Статический сайт без форм — низкий риск

## Risk Assessment

### Security
- **Риск:** Загрузка внешних ресурсов (Tailwind CDN, Google Fonts, Unsplash)
- **Смягчение:** Рекомендованные CDN, используемые в подобных проектах
- **Риск:** Контент заглушек для FAQ и Абонементов с внешних сайтов
- **Смягчение:** Временное решение для Фазы 1, на Фазе 2 контент будет управляться в админке

### Deployment
- **Риск:** Нет CI/CD пайплайна для Фазы 1
- **Смягчение:** Разработка идёт локально, пуш будет при завершении

## Backward Compatibility

- Нет изменений в существующей структуре HTML файла
- Все изменения — добавление секций и изменение контента
- Риск: минимальный

## Rollback Plan

- Git репозиторий используется для контроля версий
- При проблемах — `git revert` для отката
- Ветка `main` сохраняется чистой
